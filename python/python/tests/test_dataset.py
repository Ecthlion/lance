...
def test_data_overlay_dense(tmp_path: Path):
    ...
    overlay = lance.LanceOperation.DataOverlayFile(data_file, shared_offsets=[1, 4])
...
def test_data_overlay_newest_wins(tmp_path: Path):
    ...
    dataset = lance.LanceDataset.commit(
        ...
        lance.LanceOperation.DataOverlay(
            [
                lance.LanceOperation.DataOverlayGroup(
                    0, [lance.LanceOperation.DataOverlayFile(newer, shared_offsets=[1])]
                )
            ]
        ),
        read_version=dataset.version,
...
def test_data_overlay_sparse_per_field(tmp_path: Path):
    ...
    overlay = lance.LanceOperation.DataOverlayFile(data_file, field_offsets=[[2], [3]])
    op = lance.LanceOperation.DataOverlay(
        [lance.LanceOperation.DataOverlayGroup(0, [overlay])]
    )
    dataset = lance.LanceDataset.commit(dataset, op, read_version=dataset.version)
...
def test_data_overlay_requires_exactly_one_coverage(tmp_path: Path):
    base_dir = tmp_path / "test"
    ...
    # Neither coverage set.
    with pytest.raises(ValueError, match="exactly one of shared_offsets"):
        lance.LanceDataset.commit(
            dataset,
            lance.LanceOperation.DataOverlay(
                [
                    lance.LanceOperation.DataOverlayGroup(
                        0, [lance.LanceOperation.DataOverlayFile(data_file)]
                    )
                ]
            ),
            read_version=dataset.version,
            ...
            lance.LanceOperation.DataOverlay(
                [
                    lance.LanceOperation.DataOverlayGroup(
                        ...
                        [
                            lance.LanceOperation.DataOverlayFile(
                                data_file, shared_offsets=[0], field_offsets=[[0]]
                            )
                        ],
                    )
                ]
